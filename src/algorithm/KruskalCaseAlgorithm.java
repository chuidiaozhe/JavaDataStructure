...
public class KruskalCaseAlgorithm {
    ...
    public static void main(String[] args){
         ...
         //创建KruskalCaseAlgorithm对象实例
        KruskalCaseAlgorithm kruskalCaseAlgorithm = new KruskalCaseAlgorithm(vertex,materix);
        kruskalCaseAlgorithm.print();
        kruskalCaseAlgorithm.kruskal();

    }

    public KruskalCaseAlgorithm(char[] vertexs,int[][] matrix){
        //初始化顶点数和边的个数
    ...
    public void kruskal(){
        ...
        //按照边的权值大小进行排序（从小到大）
        sortEdge(edges);
        System.out.println("排序后图的边的集合=" + Arrays.toString(edges) + " 共"+ edges.length); //12
    ...
    private EDData[] getEdges(){
       EDData[] edData = new EDData[edgeNum];
       int index = 0;
       for (int i = 0;i < vertexs.length;i++) {
           for (int j = i+1; j < vertexs.length;j++){
               if(matrix[i][j] != INF){
                   edData[index++] = new EDData(vertexs[i],vertexs[j],matrix[i][j]);
               }
           }
       }
     ...
     * @param edges 边的集合
     * */
    private void sortEdge(EDData[] edges){
